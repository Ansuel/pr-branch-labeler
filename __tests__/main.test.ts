...
describe('PR Branch Labeler', () => {
  ...
  it('succeeds', async () => {
    ...
    const getConfigScope = nock('https://api.github.com')
      .persist()
      .get(
        `/repos/Codertocat/Hello-World/contents/.github/pr-branch-labeler.yml?ref=${main.context.payload.pull_request.head.sha}`
      )
    ...
    main.context.payload = createPullRequestOpenedFixture(
      'feature/FOO-42-awesome-stuff',
      'master',
      main.context.payload.pull_request.head.sha
    )
...
function createPullRequestOpenedFixture(
  headRef: string,
  baseRef: string,
  sha: string
) {
