...
describe('Config file loader', () => {
  ...
  it('succeeds', async () => {
    ...
    const getConfigScope = nock('https://api.github.com')
      .persist()
      .get(
        '/repos/Codertocat/Hello-World/contents/.github/pr-branch-labeler.yml?ref=0123456'
      )
  ...
  it('throws an error for an invalid config file', async () => {
    ...
    await expect(
      getConfig(octokit, 'pr-branch-labeler.yml', context)
  ...
  it('throws an error for a directory', async () => {
    ...
    await expect(getConfig(octokit, 'test', context)).rejects.toThrow(
      new Error('test is not a file.')
    )
